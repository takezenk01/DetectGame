...
これにより、プレイヤーの行動ログを出力し、上級者等が取る行動にはどのようなものが多いか等を解析したいと考えています。<br>
...
GPU を使う場合は CUDAのドライバ環境を構築する必要があります。<br>
...
nvidia-smi
```
cudaドライバ更新等は各自の環境に合わせて実施してください。

---

## 2. 仮想環境の作成と有効化（Python 3.11.9）
### 2-1 上から順番にコマンドを実行して仮想環境の作成、実行してください。
```bash
...
### 2-2 リポジトリ内にある以下のファイルを仮想環境直下に配置してください。
```bash
DetectGame/
├─ predict.py
├─ train.bat
